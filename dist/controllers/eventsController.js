...
const getEventById = (req, res, next) => {
    const { id } = req.params;
    (0, eventsModels_1.getEventByIdQuery)(id).then((rows) => {
...
const getEventsForGroup = (req, res, next) => {
    const { id } = req.params;
    (0, eventsModels_1.getEventsForGroupQuery)(id).then((rows) => {
        res.status(200).send(rows);
    });
};
...
const createEvent = async (req, res, next) => {
    ...
    try {
        const { name, description, fromGroup, location, attendees = [], scoreByMember = [], startDate, endDate, status = "INACTIVE", } = req.body;
        if (!name || !description) {
            return res
                .status(400)
...
const alterEvent = async (req, res, next) => {
    console.log("Request Body:", req.body);
    try {
        const { id, name, description, startDate, endDate, location, attendees = [], scoreByMember, } = req.body;
        const rows = await (0, eventsModels_1.alterEventQuery)(id, name, description, startDate, endDate, location, attendees, scoreByMember);
